...
def _add_gaussian_noise(projections, mean, std_dev, torch_rng):
    noise = torch.randn(projections.shape, device=projections.device, generator=torch_rng) * std_dev + mean
...
class CBProjector(Function):
    @staticmethod
    def forward(ctx, volume, vol_bbox, src_points, det_center, det_frame, det_bbox, vol_sz, det_sz, sampling_step_size):
        if volume.is_cuda:
            assert 'cb_proj_forward_cuda' in dir(tb), ("TorchBeam was compiled without CUDA support, "
                                                        "but CUDA Torch tensor given as input")
            output = tb.cb_proj_forward_cuda(volume, vol_bbox, src_points, det_center, det_frame, det_bbox, det_sz,
                                             sampling_step_size)
        else:
            output = tb.cb_proj_forward_cpu(volume, vol_bbox, src_points, det_center, det_frame, det_bbox, det_sz,
                                            sampling_step_size)
    ...
    def backward(ctx, grad_input):
        ...
        if grad_input.is_cuda:
            assert 'cb_backproj_forward_cuda' in dir(tb), ("TorchBeam was compiled without CUDA support, "
                                                            "but CUDA Torch tensor given as input")
            output = tb.cb_backproj_forward_cuda(grad_input, vol_bbox, src_points, det_center, det_frame, det_bbox,
                                                 vol_sz, 1, 1.0)
        else:
            output = tb.cb_backproj_forward_cpu(grad_input, vol_bbox, src_points, det_center, det_frame, det_bbox,
                                                vol_sz, 1, 1.0)
...
class CBBackprojector(Function):
    @staticmethod
    def forward(ctx, projection, vol_bbox, src_points, det_center, det_frame, det_bbox, vol_sz, det_sz,
                sampling_step_size):
        if projection.is_cuda:
            assert 'cb_backproj_forward_cuda' in dir(tb), ("TorchBeam was compiled without CUDA support, "
                                                            "but CUDA Torch tensor provided as input")
            output = tb.cb_backproj_forward_cuda(projection, vol_bbox, src_points, det_center, det_frame, det_bbox,
                                                 vol_sz, 1, 1.0)
        else:
            output = tb.cb_backproj_forward_cpu(projection, vol_bbox, src_points, det_center, det_frame, det_bbox,
                                                vol_sz, 1, 1.0)
        ctx.vol_bbox = vol_bbox
        ctx.src_points = src_points
        ctx.det_center = det_center
    ...
    def backward(ctx, grad_input):
        ...
        if grad_input.is_cuda:
            assert 'cb_proj_forward_cuda' in dir(tb), ("TorchBeam was compiled without CUDA support, "
                                                        "but CUDA Torch tensor given as input")
            output = tb.cb_proj_forward_cuda(grad_input, vol_bbox, src_points, det_center, det_frame, det_bbox, det_sz,
                                             sampling_step_size)
        else:
            output = tb.cb_proj_forward_cpu(grad_input, vol_bbox, src_points, det_center, det_frame, det_bbox, det_sz,
                                            sampling_step_size)
        return output, None, None, None, None, None, None, None, None
...
def norm_estimate(backproj, vol_bbox, src_points, det_centers, det_frames, det_bbox, vol_sz, detector_sz, max_iter=5,
                  multiplier=1.0, vnorm='l2'):
    with torch.no_grad():
        ...
        for i in range(max_iter):
            x = CBProjector.apply(x, vol_bbox, src_points, det_centers, det_frames, det_bbox, vol_sz, detector_sz, 1.0)
            x = CBBackprojector.apply(x, vol_bbox, src_points, det_centers, det_frames, det_bbox, vol_sz, detector_sz,
                                      1.0)
