...
def render_parallel_projection(model, rays_o, rays_d, near, far, num_samples, rand=False):
    ...
    # Broadcast to [B, P, 1]
    if near.ndim == 0: near = near.expand(B, P, 1)
    elif near.ndim == 2: near = near.unsqueeze(-1) # [B, P] -> [B, P, 1]
    
    if far.ndim == 0: far = far.expand(B, P, 1)
    elif far.ndim == 2: far = far.unsqueeze(-1)
    ...
    if num_samples > 1:
        ...
        if rand:
            ...
            steps = torch.arange(num_samples, device=device, dtype=torch.float32).reshape(1, 1, num_samples) / num_samples
