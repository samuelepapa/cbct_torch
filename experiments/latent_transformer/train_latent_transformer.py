...
from models.latent_transformer_with_coords import LatentTransformerWithCoords
from models.swin_latent_transformer import SwinLatentTransformer
...
def train(argv):
    ...
    if model_variant == "swin":
        model = SwinLatentTransformer(
            coord_dim=config.coord_dim,
            latent_dim=config.latent_dim,
            hidden_dim=config.hidden_dim,
            grid_size=getattr(config, "grid_size", 8),
            window_size=getattr(config, "window_size", 4),
            num_swin_layers=config.num_decoder_layers,  # Reuse decoder_layers as swin_layers
    ...
    elif model_variant == "coords":
        model = LatentTransformerWithCoords(
            coord_dim=config.coord_dim,
            latent_dim=config.latent_dim,
            hidden_dim=config.hidden_dim,
    ...
    if training_mode == "joint":
        joint_latents = model.init_latents(batch_size=1, device=device)
    ...
    def run_inner_loop(
        ...
        """
        inner_lr = model.get_inner_lr().item()
        ...
        for inner_step in range(num_steps):
            ...
            if fitting_mode == "projection":
                ...
                # Forward pass
                def model_fn(x):
                    if model_variant in ("coords", "swin"):
                        return model(x, latents, aabb_min, aabb_max)
                ...
                loss = F.mse_loss(pred_proj, gt_sino_batch)
                ...
                if model_variant in ("coords", "swin"):
                    pred_image_flat = model(grid_coords, latents, aabb_min, aabb_max)
                else:
    ...
    for epoch in pbar:
        ...
        for accum_step in range(grad_accum_steps):
            if fitting_mode == "projection":
                ...
                def model_fn_outer(x):
                    if model_variant in ("coords", "swin"):
                        return model(x, latents_final, aabb_min, aabb_max)
                ...
                pred_proj = (pred_proj - sino_center) / sino_half_range
                loss_outer = F.mse_loss(pred_proj, gt_sino_batch)
            elif fitting_mode == "image":
                # Image fitting outer loss on full grid
                if model_variant in ("coords", "swin"):
                    pred_image_flat = model(
                        grid_coords, latents_final, aabb_min, aabb_max
                    )  # [H*W, 1]
                else:
                    pred_image_flat = model(grid_coords, latents_final)  # [H*W, 1]
                pred_image = pred_image_flat.reshape(gt_image.shape)  # [1, 1, H, W]
                loss_outer = F.mse_loss(pred_image, gt_image)
            else:
                raise ValueError(f"Unknown fitting_mode: {fitting_mode}")
        ...
        current_inner_lr = model.get_inner_lr().item() if training_mode == "meta_learning" else 0.0
        pbar.set_postfix(
            {
                "Inner Loss": f"{avg_inner_loss:.6f}" if inner_losses else "N/A",
                "Outer Loss": f"{avg_outer_loss:.6f}",
                "LR": f"{current_lr:.2e}",
                "Inner LR": (
                    f"{current_inner_lr:.3f}" if training_mode == "meta_learning" else "N/A"
                ),
            }
        )
        ...
        if training_mode == "meta_learning":
            log_dict["train_loss_inner"] = avg_inner_loss
            log_dict["inner_learning_rate"] = current_inner_lr
        wandb.log(log_dict)
        ...
        if epoch % 10 == 0:
            with torch.no_grad():
                ...
                if training_mode == "meta_learning":
                    logger.info(f"  Inner learning rate: {model.inner_lr.item():.4f}")
                ...
                if training_mode == "meta_learning":
                    wandb_log_dict["model/inner_lr_param"] = model.inner_lr.item()
        ...
        if (epoch + 1) % config.val_interval == 0:
            ...
            if training_mode == "joint":
                ...
                val_latents = model.init_latents(1, device)
            ...
            with torch.no_grad():
                if model_variant in ("coords", "swin"):
                    rec_flat = model(grid_coords, val_latents, aabb_min, aabb_max)
                else:
                ...
                if getattr(config, "fitting_mode", "projection") == "projection":
                    ...
                    for i in range(0, num_angles, val_batch_size):
                        ...
                        def model_fn_val_sino(x):
                            if model_variant in ("coords", "swin"):
                                return model(x, val_latents, aabb_min, aabb_max)
