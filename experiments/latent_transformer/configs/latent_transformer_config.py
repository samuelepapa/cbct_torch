...
def get_config():
    ...
    config.training_mode = "meta_learning"  # or 'joint'
    ...
    config.outer_lr = 0.0005  # Learning rate for transformer Adam
    ...
    config.batch_size_angles = 4  # Views per step
    ...
    # Model variant:
    # - 'base': standard LatentTransformer
    # - 'coords': LatentTransformerWithCoords where latents are tied to 2D positions
    # - 'swin': SwinLatentTransformer with windowed attention on 2D latent grid
    config.model_variant = "swin"

    # Swin-specific params (only used when model_variant='swin')
    config.grid_size = 8  # Latent grid size (grid_size x grid_size = num_latents)
    config.window_size = 4  # Window size for windowed attention
    config.mlp_ratio = 4.0  # MLP expansion ratio in Swin blocks

    # WandB
    config.wandb = ml_collections.ConfigDict()
    config.wandb.project_name = "latent_transformer_training"
